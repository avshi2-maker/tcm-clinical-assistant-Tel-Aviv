...
try:
    ...
    try:
        ...
        USE_NEW_SDK = False
        print("⚠️  Using deprecated google.generativeai (works but outdated)")
    except ImportError:
...
def initialize_clients():
    """Initialize Supabase and Gemini clients with error handling."""
    ...
    try:
        ...
        print("   3. Make sure Supabase URL is correct")
        exit(1)
    ...
    try:
        ...
        print("❌ ERROR: Cannot initialize Gemini!")
        print(f"   {e}")
        exit(1)
...
def translate_point_batch(point_data: dict, point_number: int, total_points: int, model_client) -> dict:
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Call Gemini API (works with both old and new SDK)
            if USE_NEW_SDK:
            ...
            # Handle network errors
                ...
                if attempt < max_retries - 1:
                    ...
                    print(f"  ❌ Network error persists. Check your internet connection.")
                    return None
...
def save_translation_to_db(supabase_client, point_id: int, translated_fields: dict):
    """Save translated fields to database."""
    try:
        # Update the point with Hebrew translations
        response = supabase_client.table('dr_roni_complete').update(
            translated_fields
...
def main():
    ...
    # Initialize clients
    supabase, model_client = initialize_clients()
    print()
    ...
    try:
        ...
        print(f"❌ ERROR: Cannot load points from database!")
        print(f"   {e}")
        print()
    ...
    for i, point in enumerate(points_to_translate, 1):
        try:
            ...
            translated_fields = translate_point_batch(point, i, len(points_to_translate), model_client)
            
            if translated_fields:
                # Save to database
                success = save_translation_to_db(supabase, point['id'], translated_fields)
